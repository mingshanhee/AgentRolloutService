...
def create_app(runner: BaseRunner, environments: Dict[str, Dict[str, Any]] = {}) -> FastAPI:
    ...
    def start_instance(request: StartInstanceRequest):
        try:
            # Check if container_id maps to a pre-defined environment
            if request.container_id in environments:
                env_config = environments[request.container_id]
            ...
            instance_id = runner.start_instance(request.container_id, env_config)
    ...
    def execute_command(request: ExecuteCommandRequest):
        try:
            result = runner.execute_command(request.container_id, request.cmd)
    ...
    def close_instance(request: CloseInstanceRequest):
        try:
            runner.close_instance(request.container_id)
