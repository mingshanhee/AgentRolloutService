...
class SlurmRunner(BaseRunner):
    ...
    def start_instance(self, instance_id: str, environment_config: Dict[str, Any]) -> str:
        ...
        # If an image is provided, we might wrap it (omitted for brevity, assuming simple usage)
        # Simplified: just submitting 'sleep infinity'
        # In a real scenario, this would potentially use 'srun --pty' or similar within the batch script
        # or use singularity exec instance://...
        ...
        try:
            ...
            logger.info(f"Started Slurm job {job_id} for instance {instance_id}")
            return instance_id
    ...
    def execute_command(self, instance_id: str, cmd: str) -> Dict[str, Any]:
        ...
        job_id = self.running_instances[instance_id]["job_id"]
        ...
        try:
            ...
            logger.error(f"Failed to execute command in job {job_id}: {e}")
            raise
    ...
    def close_instance(self, instance_id: str) -> None:
        ...
        job_id = self.running_instances[instance_id]["job_id"]
        resources = self.running_instances[instance_id]["resources"]
